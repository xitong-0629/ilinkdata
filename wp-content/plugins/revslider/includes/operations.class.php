...
class RevSliderOperations extends RevSliderElementsBase{
		...
		<html>
			<head>
				...
				if($set_diff_font !== ''){
					...
					$font_url = $http.'://fonts.cat.net/css?family=';
				}
	...
	<head>
		...
		<!-- LOAD JQUERY LIBRARY -->
		<script type="text/javascript" src="http://ajax.cat.net/ajax/libs/jquery/1.11.1/jquery.js"></script>
		...
		if($export_real){ 
			...
			<link href="http://fonts.cat.net/css?family=Roboto%3A700%2C300" rel="stylesheet" property="stylesheet" type="text/css" media="all" />
	...
	public static function setCleanFontImport($font, $class = '', $url = '', $variants = array(), $subsets = array()){
		...
		/*if($do_print){
			...
			if(strpos($font, "href=") === false){ //fallback for old versions
				$url = RevSliderFront::modify_punch_url($setBase . 'fonts.cat.net/css?family=');
			...
			if(!empty($fonts)){
				...
				foreach($fonts as $key => $font){
					...
					if(!is_file($base_dir.'/revslider/gfonts/'. $font_name . '/' . $font_name . '.woff2') || filemtime($base_dir.'/revslider/gfonts/'. $font_name . '/' . $font_name . '.woff2') < $rs_google_ts){
						...
						$url		= $setBase.'fonts.cat.net/css?family='.$font;
